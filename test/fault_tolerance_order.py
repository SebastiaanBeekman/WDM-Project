...
class TestMicroservices(unittest.TestCase):
    ...
    def test_order_contains_no_faulty_logs(self):
        """Test logging of all functions in order-microservice"""
    ...
    def test_order_create_contains_faulty_log(self):
        """Test if the fault tolerance recovery works with `create_order`"""
        # Get initial log count
        order_log_count = int(tu.get_order_log_count())
    ...
    def test_order_find_contains_faulty_log(self):
        """
        Make a faulty log for `find_order` and see if the `fix_fault_tolerance` function fixes it
        obsolete, because read operation is fine
        """
        # Get initial log count
        order_log_count = int(tu.get_order_log_count())
        self.assertIsNotNone(order_log_count)
    ...
    def test_order_add_item_contains_faulty_log(self):
        """Test fault tolerance for a crash in `add_item`"""
        # Get initial log count
        order_log_count = int(tu.get_order_log_count())
        self.assertIsNotNone(order_log_count)
        ...
        for i in range(4):
            ...
            if i >= 3:
                log5_resp = tu.create_order_log(
                    ...
                    new_ordervalue=OrderValue(user_id=user_id, paid=False, items=[
                                              (item_id, int(quantity))], total_cost=price*quantity),
                )
